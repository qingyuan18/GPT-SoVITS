...
def get_tts_wav(ref_wav_path, prompt_text, prompt_language, text, text_language,output_s3uri):
    ...
    # logger.info("%.3f\t%.3f\t%.3f\t%.3f" % (t1 - t0, t2 - t1, t3 - t2, t4 - t3))
        if stream_mode == "normal":
            ...
            ##opt2: pack indivitual wav ,write to s3 first , and return s3 file path
            chunked_audio = pack_wav(audio_bytes,hps.data.sampling_rate)
            result = write_wav_to_s3(chunked_audio.getvalue(),output_s3uri)
